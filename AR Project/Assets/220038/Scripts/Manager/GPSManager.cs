...
public class GPSManager : MonoBehaviour
{
    ...
    [DllImport("__Internal")]
    private static extern void WatchDeviceorientation();//Plugins内のDeviceAngleから取得してる
    private void Start()
    {
        ;//Invoke(nameof(GetGPS), 0.1f);//1秒後に取得テスト実行
    }
    public void GetGPS()
    {
        GetCurrentPosition();//GPSManager内のを呼び出す
        WatchDeviceorientation();//DeviceAngle内のを呼び出す
    }
    ...
    public void ShowRotation(string rotation)
    {
        ...
        get_xrot = float.Parse(rotations[0]);
    }
}
